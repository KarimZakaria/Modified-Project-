...
function  searchnote(term){
	...
	for(var i=0 ;i<notecontainer.length ;i++){
		...
		if(notecontainer[i].name==term){
			 ...
			 searchcol+='  <div class="col-md-3"><div class="product"><h3> '+notecontainer[i].name+'</h3><p>'+notecontainer[i].text+'</p> <button onclick="deletnote('+i+')"class="btn btn-primary"> Delete</button></div></div>'
        }
		}
